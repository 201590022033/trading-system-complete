...
def preview(registry,instrument_id,decision,bar,session,gates,schedule,admission,now,units=1):
    ...
    if not instrument.enabled:reasons.append('DISABLED_INSTRUMENT')
    ...
    if now<decision.decision_time or bar.available_time>decision.decision_time or now-bar.event_time>__import__('datetime').timedelta(seconds=bar.source.max_age_seconds) or bar.stale:reasons.append('STALE_OR_FUTURE_DATA')
    ...
    if decision.score is None or decision.score==0:reasons.append('NO_DIRECTIONAL_SIGNAL')
