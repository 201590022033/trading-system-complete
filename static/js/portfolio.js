...
function renderPaper(data) {
  ...
  $('#paper-pending').innerHTML=data.pending?.length ? data.pending.map(p=>`<div class="decision-row"><div><b>${esc(p.instrument_id)}</b><small>Rank ${esc(p.rank)} · ${esc(p.direction)} · ${esc(when(p.evaluated_at))}</small></div><span class="status-chip">${esc(sentence(controls.paused?'PAUSED':reasons.get(p.instrument_id) || p.state))}</span></div>`).join('') : emptyPaper('No eligible proposals are queued yet. Check the latest data and canonical ranking.');
